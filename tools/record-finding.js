...
// cve/cves (CVE id(s)), epss (0-1). If cve/cves are present and epss is absent, epss is
// auto-filled (cached-live FIRST-EPSS score when tools/threatintel.js has refreshed it, else
...
function record(raw) {
  ...
  fs.appendFileSync(OUT(), JSON.stringify(f) + '\n');
  return Object.assign({ ok: true, recorded: key(f) }, syncToFindingsTab(f));
}
...
module.exports = { record, recordWithVerify, validate, key, redact, vaultSecret, normalizeCves, enrichEpss, enrichCveIntel, standardsFor, registryClasses, syncToFindingsTab, VERIFY_LEVELS, SEVERITY_CEILING, severityAllowedAt };
